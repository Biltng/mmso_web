    ...
    <div className="pt-[4.5rem] lg:pt-[5rem]">
      <section className="relative bg-gradient-to-br from-graphite-900 via-graphite-800 to-graphite-900 py-16 lg:py-24 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white mb-4">Property <span className="text-brand-400">Development</span></h1>
          <p className="text-lg text-steel-300 max-w-2xl mx-auto leading-relaxed">From concept to completion and beyond, our development division delivers exceptional property outcomes across every stage of the lifecycle.</p>
        </div>
      </section>
      ...
      <section className="relative pt-6 pb-14 bg-white overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-10">
            <h2 className="text-3xl font-bold text-graphite-900 mb-3">5 End-to-End <span className="text-brand-500">Development Services</span></h2>
            <p className="text-steel-500 leading-relaxed text-sm">We support property and construction projects through every phase.</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-6 gap-5">
            {services.map((service, idx) => (
              <Link key={idx} to={`/services/${service.slug}`} className={`group p-5 rounded-2xl bg-graphite-50 border border-steel-200 hover:border-brand-200 hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 lg:col-span-2 ${idx === 3 ? "lg:col-start-2" : ""} ${idx === 4 ? "lg:col-start-4" : ""}`}>
                ...
                <p className="text-sm text-steel-500 leading-relaxed mb-3">{service.description}</p>
                <div className="flex flex-wrap gap-1.5">
                  {service.features.map((feature, fIdx) => (
                    <span key={fIdx} className="text-[11px] px-2 py-0.5 bg-white group-hover:bg-brand-50 text-steel-500 group-hover:text-brand-700 rounded transition-colors">{feature}</span>
                  ))}
                </div>
              </Link>
            ))}
      ...
      <section className="relative py-14 bg-gradient-to-br from-graphite-900 via-graphite-800 to-graphite-900 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-2xl sm:text-3xl font-bold text-white mb-3">Ready to Start Your Next Property Project?</h2>
          <p className="text-sm text-steel-300 mb-6 max-w-xl mx-auto">Let's discuss how we can support your development, construction, and property management goals.</p>
          <div className="flex flex-wrap gap-3 justify-center">
            <button type="button" onClick={goToContact} className="inline-flex items-center gap-2 px-7 py-3 bg-gradient-to-r from-brand-500 to-brand-600 hover:from-brand-600 hover:to-brand-700 text-white font-semibold rounded-full shadow-lg shadow-brand-500/25 transition-all duration-300 text-sm">
